function FormViewModel(data) {
    ...
    self.removeCharm = function (obj) {
        alertify.confirm("Xác nhận", "Bạn có chắc chắn muốn xóa?",
            function () {
                ...
                $.ajax({
                    url: data.API_URLs.DeleteCharm,
                    type: "POST",
                    data: { id: obj.id },
                    success: function (response) {
                        if (response.IsSuccess == true) {
                            self.Charms.remove(obj);
    ...
    self.saveEdit = function () {
        var _url = IsEditMode == true ? data.API_URLs.EditCharm : data.API_URLs.CreateCharm;
