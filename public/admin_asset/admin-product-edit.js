function FormViewModel(data) {
    ...
    self.topics = ko.observable(data.Topics || 0);
    ...
    self.categoryId.subscribe(function(value){
        if(value){
            var seletedItem = self.categories().filter(function(i){ return i.id == value; })[0];
    ...
    self.SelectedPiece = ko.observable(self.pieces().filter(function(i){ return i.id == self.pieceId(); })[0] || null);

    self.pieceId.subscribe(function(value){
        if(value){
            var seletedItem = self.pieces().filter(function(i){ return i.id == value; })[0];
            if(seletedItem){
    ...
    self.topicId = ko.observable(data.Product.topic_id || null);

    self.quantityOfPieces = ko.observable(data.Product.quantity_of_pieces || 1);

    self.price = ko.computed(function() {
      if(self.SelectedPiece() && self.quantityOfPieces())
        return parseInt(self.SelectedPiece().price) * parseInt(self.quantityOfPieces());
        else return 0;
    });
